...
def get_Tacotron2():
    ...
    checkpoint_path = os.path.join(os.path.join(
        "Tacotron2", "pre_trained_model"), checkpoint_path)
...
if __name__ == "__main__":
    # Test
    mel_gt = np.load(os.path.join(
        hparams.mel_ground_truth, "ljspeech-mel-00001.npy")).T
    mel_tac2 = np.load(os.path.join(hparams.mel_tacotron2, "0.npy"))

    txt = "Printing, in the only sense with which we are at present concerned, differs from most if not from all the arts and crafts represented in the Exhibition"
    tacotron2 = get_Tacotron2()
    mel, _, _ = load_data_from_tacotron2(txt, tacotron2)

    plot_data([mel_gt, mel_tac2, mel])
